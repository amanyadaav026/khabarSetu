...
const HeroSection = ({ article, latestArticles }) => {
  ...
  return (
    <section className="mt-10">
      ...
      {/* Top Layout */}
      <div className="grid gap-6 lg:grid-cols-[1.8fr_0.8fr] lg:gap-5">
        ...
        <div className="relative overflow-hidden rounded-2xl">
          ...
          <div className="absolute bottom-0 left-0 z-10 p-5 sm:p-7 lg:p-12">
            ...
            <h1
              onClick={() => navigate(`/article/${article._id}`)}
              className="mt-3 max-w-4xl cursor-pointer text-2xl font-black leading-tight text-white transition hover:text-red-300 sm:mt-4 sm:text-4xl lg:mt-5 lg:text-5xl"
            >
              {article.title}
            </h1>
            
            <p className="mt-3 max-w-3xl text-sm leading-6 text-gray-200 sm:mt-4 sm:text-base sm:leading-7 lg:text-lg lg:leading-8">
              {article.summary}
            </p>
            
            <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-2 text-xs text-gray-300 sm:mt-6 sm:gap-4 sm:text-sm">
            ...
            >
              <div className="flex gap-4">
                <img
                  src={item.imageUrl}
                  alt={item.title}
                  className="h-20 w-20 shrink-0 rounded-xl object-cover transition-transform duration-300 group-hover:scale-105 sm:h-24 sm:w-24"
                />
                ...
                <div className="flex-1">
                  ...
                  <h3 className="mt-2 text-sm font-bold leading-6 text-slate-900 transition-colors duration-300 group-hover:text-red-600 sm:mt-3 sm:text-lg sm:leading-7">
                    {item.title}
                  </h3>
